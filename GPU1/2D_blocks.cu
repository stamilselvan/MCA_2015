...
__global__ void calc_fractal(int width, int height, float c_re, float c_im, unsigned char* dest) {
	...
	int index = col + row * (blockDim.x * gridDim.x); /* Varies from 0 to 65536 */

	int x = index % IMG_SIZE;
	int y = index / width;
	
	float f_x = (float)(x*0.8) / (float)(width)-0.8;
	float f_y = (float)(y*0.8) / (float)(height)-0.8;
	dest[index]=iterate_pixel(f_x,f_y, c_re, c_im);
...
int main(void) {
	...
	dim3 dimGrid(IMG_SIZE/BLOCK_DIM_X, IMG_SIZE/BLOCK_DIM_Y);
	...
	calc_fractal<<<dimGrid, dimBlock>>> (256, 256, 0.28, 0.008, dev_img);
